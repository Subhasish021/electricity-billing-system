...
public class UpdateCustomer extends JFrame implements ActionListener
{
    ...
    UpdateCustomer(String meterno)
    {
     ...
     tfphnumber.setBounds(230,300,200,20);
       ...
       {
          Conn c=new Conn();
          ResultSet rs = c.s.executeQuery("select * from customer2 where meterno = '"+meterno+"'");
          while(rs.next())
          {
    ...
    public void actionPerformed(ActionEvent ae)
    {
        if(ae.getSource()==update)
        {
            ...
            try{
                Conn c=new Conn();
                c.s.executeUpdate("update customer2 set adress='"+address+"', city='"+city+"', state='"+state+"', email='"+Email+"', number='"+Phnumber+"' where meterno='"+meterno+"' ");
